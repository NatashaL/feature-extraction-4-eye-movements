function result = dispersionThresholdIdentification2( data )
...
while i + minwindowsize*2 <= size(data,2)
    ...
    disp = dispersion2(currentpoints);
    ...
    if windowsize <= minwindowsize
        %fprintf('didnt find a fixation');
        i = i + 2;
        continue 
    else
        %fprintf('found a fixation');
        xx = mean(currentpoints(1:2:end-2));
        yy = mean(currentpoints(2:2:end-2));
        rezx = cat(2,rezx,[xx]);
        rezy = cat(2,rezy,[yy]);
        Dispersion2 = cat(2,Dispersion2,[disp]);
...
result = getAttrInStruct(rezx,rezy,dispersion1,Dispersion2,duration,velocity,areaOfFixation,saccadeAmplitudes,acceleration,decceleration );
